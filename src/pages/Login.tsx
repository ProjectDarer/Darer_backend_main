...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[var(--cs-magenta)]/10 blur-[100px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mt-4">Log in to DARER</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link to="/forgot-password" className="text-sm text-[var(--cs-cyan)] hover:text-[var(--cs-green)] hover:underline transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative group">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground group-focus-within:text-[var(--cs-cyan)] transition-colors" />
                <Input
                  type={showPassword ? 'text' : 'password'}
                  placeholder="Enter your password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="pl-10 pr-10 bg-black/20 border-border focus-visible:ring-[var(--cs-cyan)]"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-[var(--cs-cyan)] transition-colors"
                >
        ...
        <p className="text-center mt-6 text-sm text-muted-foreground">
          ...
          <Link to="/signup" className="text-[var(--cs-cyan)] hover:underline font-medium hover:text-[var(--cs-green)]">
            Sign up
          </Link>
        </p>
      </div>
